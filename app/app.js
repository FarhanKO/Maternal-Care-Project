...
// The Guardian web companion is also available from the same LAN origin as
// the API. This keeps browser invitations usable even when nobody has a
// separate Vite process listening on port 5174.
const GUARDIAN_DIR = path.join(__dirname, 'guardian-app', 'dist');
const hasGuardian = fs.existsSync(path.join(GUARDIAN_DIR, 'index.html'));
if (hasGuardian) {
  app.use('/guardian-app', express.static(GUARDIAN_DIR));
}
...
if (hasGuardian) {
  app.get('/guardian-app', (req, res) => {
    res.sendFile(path.join(GUARDIAN_DIR, 'index.html'));
  });
  app.get('/guardian-app/*', (req, res, next) => {
    if (req.path.includes('.')) return next();
    return res.sendFile(path.join(GUARDIAN_DIR, 'index.html'));
  });
}
