...
A small FastAPI service in front of the model trained by train/train.py. The Node
...
def predict(r: Reading) -> Prediction:
    if _model is None:
        raise HTTPException(status_code=503, detail="Model not loaded — run train/train.py")
